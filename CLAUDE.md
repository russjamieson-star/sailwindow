...
**Pricing/entitlement backend built and wired (2026-07-21), deploy pending.** `subscription-service/` (Express + Stripe webhooks + Firestore) now reads which edition(s) a subscription unlocks from each Stripe Price's own `editions` metadata — so the 4-edition combinatorics (any-two bundles, Trio, Grand Loop All-Access) are a Stripe Dashboard edit, not a code change. Client-side (`shared/engine.js`, mirrored into every dist folder) now really verifies via the backend instead of trusting an honor-system button click, and silently re-checks on every load so a lapsed subscription locks back down on its own. Still needs Russ to actually run the GCP deploy (`subscription-service/DEPLOY.md`, manual — GCP/Stripe account access) and paste the resulting URL into `backendUrl` (4 files, currently `"#"`); until then the client falls back to the old honor-system flow automatically. See TODO.md's "Gulf + Atlantic bundle" section.
...
- **Waitlist backend (added 2026-07-20) — shared across all future editions.** One Google Sheet ("SailWindow — Waitlist", under russjamieson@gmail.com, id `1pnTDkLUuZNSfu8kJ9U3QKLvvxf--tShnWcDgPO02nco`) + one Apps Script `doPost()` (see `waitlist-Code.gs`) serve every edition's waitlist, not just Atlantic's — each row is tagged with an `Edition` column. The form posts JSON via `fetch()` (not a native form submit, which would both fail `JSON.parse` server-side and redirect the visitor off-site to a bare Apps Script response page) using `Content-Type: text/plain` specifically to dodge a CORS preflight Apps Script Web Apps can't handle. Client-side function `submitWaitlist(event, edition)` (still defined, shared engine) is edition-parameterized on purpose: adding a Great Lakes or Mississippi waitlist later is copy the same form block, change `id="waitlist-form-atlantic"` → `-greatlakes`/`-mississippi` and the `submitWaitlist(event,'atlantic')` argument — no new script, no new sheet, no new deploy. `WAITLIST_SCRIPT_URL` is a `"#"` placeholder until the Apps Script is deployed as a Web App and the URL pasted in — see TODO.md for the manual deploy steps (can't be done from here; Apps Script deployment requires the Apps Script editor UI).
  - **Gulf's own entry point to this backend was removed 2026-07-31** (see "Coming Next" bullet below) — the backend/Sheet/Apps Script are untouched and still live, but `dist/index.html` no longer posts to them. Worth confirming with Russ whether that's permanent or the forms should come back in a future revision.
- **Hero visual pass (2026-07-31).** Following on the 2026-07-28 "Ask Captain Jim" rebrand, this session made a second pass at the Hero specifically:
  - **Brand lockup enlarged:** `.hero-wordmark` grown from 1.4rem/800 to 3.4rem/900 (line-height 1), `.hero-logo-img` from 68px to 112px, `.hero-brand` gap/margin bumped to match — the "Ask Captain Jim" logo/wordmark is now the dominant element on load, not a small corner detail.
  - **Primary CTA recolored:** `.btn-hero-primary` changed from teal (`var(--primary)`) to sailing orange `#f26a3d` / white text, hover `#d9532a` — a deliberate one-off departure from the teal used everywhere else in the app, to make the single most important button stand out. (See Hero CTAs section for the copy change.)
  - **Drawbridges/Cruiser's Log cards unified:** removed the `.hw-card.hw-bridge-card` / `.hw-card.hw-log-card` warn/gold background+border+tag overrides and switched their `hwi-lg` icon class from `c-warn`/`c-gold` to `c-primary` — all 6 right-column feature cards now render identically (white card, teal icon/tag). The small left-column feature-grid chips for these same two items were deliberately left as `c-warn`/`c-gold` — only the big cards were in scope.
  - **"Coming Next" simplified:** replaced the `#coming-next-waitlist` section (cards + email forms) with one static bold-black line: "Coming Next + Great Lakes Edition + Mississippi Edition" — no inputs, no buttons, no links. See the waitlist-backend bullet above for the consequence.
  - **The Backstory box added:** new section directly below "Coming Next" — founder/crew backstory (four Emmy-winning documentary journalists who've sailed with Captain Jim for decades; Russ as cook/unofficial-second-in-command who built and named the app) plus a "Powered by Data. Guided by Judgment." safety-disclaimer subsection ("sailing always carries risk... you can always say no... sail safely"). Static content, no JS, no new DOM ids anything else depends on.
  - All of the above were prototyped first as a standalone Claude Artifact (mockup of just the Hero section, JS stripped) for visual review, then hand-merged back into the real `dist/index.html` (onclick handlers, relative logo path, and everything outside the Hero left untouched) once approved.

## Hero CTAs
- `heroStartTrial()` — "Sailing Today? Ask Captain Jim →" — orange `#f26a3d` pill (changed from teal 2026-07-31, hover `#d9532a`); copy shortened from "Sailing Window?" same day. Predates that: "Check Your Window — Free for 7 Days →" before the 2026-07-28 "Ask Captain Jim" rebrand.
- `heroSubscribe()` — "Subscribe Now — Free for 7 Days" (teal outline button; text gained the "— Free for 7 Days" suffix in the 2026-07-28 rebrand, previously just "Subscribe Now")
...
- **Shared entitlement backend built and wired, not yet deployed (2026-07-21).** See the Long-Range Roadmap section above — this is the mechanism a future in-app edition switcher would rely on to trust that a bundle subscriber really is entitled to the edition they're switching to.
